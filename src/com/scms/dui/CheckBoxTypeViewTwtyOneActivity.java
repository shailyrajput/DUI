...
public class CheckBoxTypeViewTwtyOneActivity extends Activity implements
		OnClickListener {
	private Activity activity = CheckBoxTypeViewTwtyOneActivity.this;
	...
	private int postion;
	private ArrayList<String> twentyOneDatalist;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		twentyOneDatalist = new ArrayList<String>();
	...
	private void setview(int postion) {
		...
		System.out.println("=== himani activity number :" + activtiy_no);
	...
	private void conditionCheck() {
		...
		map = singletonClass.getMap().get(QuestionUtiltiy.Q21);
	...
	public void onClick(View v) {
		...
		case R.id.check_btn_one:
			...
			if (check_btn_one.isChecked()) {
				...
				check_one = check_btn_one.getText().toString();
				map.put(QuestionUtiltiy.Q22_1, QuestionUtiltiy.Q22_5);
			...
			singletonClass.map.put(optionlist.get(0).getOptionText(), check_one);
				...
				map.remove(QuestionUtiltiy.Q22_1);
			}
			break;
		...
		case R.id.check_btn_two:
			if (check_btn_two.isChecked()) {
				...
				map.put(QuestionUtiltiy.Q22_2, QuestionUtiltiy.Q22_2);
			} else {
				valSetOne.remove(check_two);
				singletonClass.map.remove(optionlist.get(1).getOptionText());
				
				map.remove(QuestionUtiltiy.Q22_2);
		...
		case R.id.check_btn_four:
			...
			if (check_btn_four.isChecked()) {
				...
				map.put(QuestionUtiltiy.Q22_4, QuestionUtiltiy.Q22_4);
				
			} else {
				valSetOne.remove(check_four);
			
				singletonClass.map.remove(optionlist.get(3).getOptionText());
				map.remove(QuestionUtiltiy.Q22_4);
			}
			break;
		case R.id.check_btn_five:
			...
			if (check_btn_five.isChecked()) {
				...
				map.put(QuestionUtiltiy.Q22_5, QuestionUtiltiy.Q22_5);
				...
				map.remove(QuestionUtiltiy.Q22_5);
			}
			break;
		...
		case R.id.check_btn_six:
			...
			if (check_btn_six.isChecked()) {
				...
				map.put(QuestionUtiltiy.Q22_6,QuestionUtiltiy.Q22_6);
			} else {
				valSetOne.remove(check_six);
				singletonClass.map.remove(optionlist.get(0).getOptionText());
				map.remove(QuestionUtiltiy.Q22_6);
			}
			break;
		default:
			break;
	...
	private void nextBtnAction() {
		...
		singletonClass.addMap(main_question, map);
      	System.out.println("===21 data"+singletonClass.getMap().get(QuestionUtiltiy.Q21));
		postion = Integer.parseInt(activtiy_no);
		
		Intent intent = new Intent(CheckBoxTypeViewTwtyOneActivity.this,CheckBoxTypeViewTwtyTwoActivity.class);
		overridePendingTransition(0, 0);
		intent.putExtra("Position", postion - 1);
		intent.putExtra("saveposition", savePosition + 1);
		intent.putExtra("back_activity","21");
		startActivity(intent);
		finish();
